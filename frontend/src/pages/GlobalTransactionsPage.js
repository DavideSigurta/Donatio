...
import { GlobalTransactionHistory } from "../components/transaction/GlobalTransactionHistory";
import { useWeb3 } from "../contexts/Web3Context";

export function GlobalTransactionsPage() {
  const { globalTransactions, globalTransactionsLoading } = useWeb3();
  
  // Stato per i filtri
  const [typeFilter, setTypeFilter] = useState('all'); // 'all', 'buy', 'sell', 'deposit', 'donation'
  ...
  const filteredTransactions = useMemo(() => {
    if (!globalTransactions || globalTransactionsLoading) return [];
  ...
  }, [globalTransactions, typeFilter, periodFilter, globalTransactionsLoading]);
    ...
    <div>
      <nav aria-label="breadcrumb">
        <ol className="breadcrumb">
          ...
          <li className="breadcrumb-item active" aria-current="page">Tutte le Transazioni</li>
        </ol>
      </nav>

      <h1 className="mb-4">Tutte le Transazioni <span className="text-white badge bg-primary">Admin</span></h1>
      <p className="text-muted mb-4">
        Qui puoi visualizzare tutte le transazioni effettuate sulla piattaforma da tutti gli utenti.
      </p>
      ...
      {/* Passiamo le transazioni filtrate al componente */}
      <GlobalTransactionHistory 
        customTransactions={filteredTransactions} 
        customIsLoading={globalTransactionsLoading} 
      />
    </div>
  );
}